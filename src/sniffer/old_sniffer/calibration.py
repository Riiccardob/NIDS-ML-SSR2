...
def analyze_csv(csv_path: Path, sample_size: int = 10000) -> Dict:
    ...
    try:
        ...
        if missing:
            print(f"  Feature mancanti: {missing[:5]}")
        else:
            print(" Tutte le feature del modello presenti")
    except Exception as e:
        print(f"  Errore caricamento artifacts: {e}")
        selected_features = []
...
def compare_stats_pcap_csv(pcap_path: Path, csv_path: Path, max_packets: int = 500000) -> Dict:
    ...
    if len(valid_flows) < 100:
        print("  Troppi pochi flussi validi per statistiche affidabili")
...
def main():
    ...
    if args.check_coverage:
        print("\n  --check-coverage non disponibile!")
