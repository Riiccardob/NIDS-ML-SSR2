...
Training di classificatore XGBoost per Network Intrusion Detection.

XGBOOST (eXtreme Gradient Boosting):
------------------------------------
Algoritmo di gradient boosting ottimizzato che:
- Addestra alberi in sequenza, ognuno corregge errori del precedente
- Usa regolarizzazione L1/L2 per prevenire overfitting
- Supporta parallelizzazione e GPU
- Gestisce valori mancanti nativamente

Vantaggi:
- Eccellente performance su dati tabulari
- Veloce in training e inference
- Gestisce sbilanciamento con scale_pos_weight
- Early stopping previene overfitting

Svantaggi:
- Piu sensibile a iperparametri rispetto a RF
...
Opzioni disponibili:
    ...
    --early-stopping        Abilita early stopping (default: abilitato)
...
ESEMPI:
-------
# Training standard con early stopping
python src/training/xgboost_model.py

# Senza early stopping
python src/training/xgboost_model.py --no-early-stopping

# Training multiclasse con piu iterazioni
python src/training/xgboost_model.py --task multiclass --n-iter 50
...
def train_xgboost(X_train: pd.DataFrame,
    ...
    """
    Addestra XGBoost con RandomizedSearchCV e early stopping opzionale.
    
    Processo:
    1. RandomizedSearchCV trova migliori iperparametri
    2. Se early_stopping=True, riaddestra con piu alberi e stop automatico
    3. Valuta su validation set
    ...
    Args:
        ...
        n_iter: Combinazioni iperparametri da testare
        cv: Fold cross-validation
        use_early_stopping: Se True, usa early stopping nel training finale
        n_jobs: Core CPU
        random_state: Seed
    ...
    # Calcola peso per classe sbilanciata (solo binary)
    ...
    if scale_pos_weight and task == 'binary':
    ...
    # Early stopping: riaddestra con piu alberi
    best_iteration = None
...
def save_model(model: XGBClassifier,
    ...
    task = results['task']
    model_path = output_dir / f"xgboost_{task}.pkl"
    results_path = output_dir / f"xgboost_{task}_results.json"
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Training XGBoost per NIDS',
