...
def analyze_csv(csv_path: Path, sample_size: int = 5000) -> Dict:
    ...
    try:
        ...
        print(f"\n  Impossibile caricare artifacts: {e}")
        selected_features = []
        scaler_columns = []
    ...
    if missing_critical:
        print(f"\n  Feature CRITICHE mancanti nel CSV:")
    ...
    if missing_model:
        print(f"\n  Feature del MODELLO mancanti nel CSV:")
...
def compare_pcap_csv(pcap_path: Path, csv_path: Path, num_flows: int = 30) -> Dict:
    ...
    if not src_col or not dst_col:
        print("\n  ERRORE: Colonne IP non trovate nel CSV!")
    ...
    if matches_found == 0:
        print("\n  Nessuna corrispondenza trovata tra PCAP e CSV!")
    ...
    if comparisons:
        ...
        if bad > total * 0.3:
            print(f"\n  ATTENZIONE: Molte feature disallineate!")
...
def check_pcap_csv_coverage(pcap_path: Path, csv_paths: List[Path]) -> Dict:
    ...
    for csv_path in csv_paths:
        if not csv_path.exists():
            print(f"    Non trovato: {csv_path.name}")
            continue
        ...
        if src_col and dst_col:
            ...
            print(f"    Colonne IP non trovate in {csv_path.name}")
    ...
    if len(csv_flows) > 0:
        ...
        if coverage > 80:
            ...
            print("  Copertura parziale - alcuni flussi CSV non sono nel PCAP")
        else:
            print("  Bassa copertura - il PCAP potrebbe non corrispondere ai CSV")
    else:
        print("\n  Nessuna coppia IP trovata nei CSV!")
        coverage = 0
